...
export async function createProduct(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (err.code === 11000) {
      ...
      errorResponse(res, err, "Failed to create product", StatusCodes.BAD_REQUEST);
    }
  }
}
...
export async function getProducts(req: Request, res: Response): Promise<void> {
  try {
    const parsedReq = req as RequestWithParsedQuery<ProductQuery>;
    const {
      limit = 10,
      page = 1,
      sortBy = "createdAt",
    ...
    if (availability) filter.stock = availability === "inStock" ? { $gt: 0 } : 0;

    const [products, total] = await Promise.all([
      productModel.find(filter).limit(limit).skip((page - 1) * limit).sort({ [sortBy]: sortOrder }).lean(),
      productModel.countDocuments(filter),
    ]);

    paginatedResponse(res, products, { total, page, limit, totalPages: Math.ceil(total / limit) }, "Products fetched successfully");
  } catch (error) {
...
export async function updateProduct(req: Request, res: Response): Promise<void> {
  try {
    const product = await productModel.findByIdAndUpdate(toObjectId(req.params.id), { $set: req.body }, { new: true });
    if (!product) {
      errorResponse(res, null, "Product not found", StatusCodes.NOT_FOUND);
      return;
    }
...
export async function deleteProduct(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (activeOrders > 0) {
      errorResponse(res, null, `Cannot delete product with ${activeOrders} active order(s)`, StatusCodes.BAD_REQUEST);
      return;
    }
    const product = await productModel.findByIdAndDelete(toObjectId(req.params.id));
    if (!product) {
      errorResponse(res, null, "Product not found", StatusCodes.NOT_FOUND);
      return;
    }
...
export async function getProductRatings(req: Request, res: Response): Promise<void> {
  try {
    const ratings = await rateModel.find({ productId: toObjectId(req.params.id) }).populate("ratedBy", "firstName lastName avatar").sort({ createdAt: -1 });
...
export async function addProductRating(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!hasPurchased) {
      errorResponse(res, null, "You can only rate products that you have purchased", StatusCodes.FORBIDDEN);
      return;
    }

    const existingRating = await rateModel.findOne({ ratedBy: userId, productId: toObjectId(productId) });
    let ratingDoc;
    if (existingRating) {
      ratingDoc = await rateModel.findByIdAndUpdate(existingRating._id, { $set: { rating } }, { new: true });
    } else {
      ratingDoc = await rateModel.create({ ratedBy: userId, productId: toObjectId(productId), rating });
    }

    const allRatings = await rateModel.find({ productId: toObjectId(productId) });
    const avgRating = allRatings.reduce((sum, r) => sum + r.rating, 0) / allRatings.length;
    await productModel.findByIdAndUpdate(productId, { $set: { avgRating: Math.round(avgRating * 10) / 10, ratingCount: allRatings.length } });

    successResponse(res, ratingDoc, existingRating ? "Rating updated" : "Rating added");
  } catch (error) {
    logger.error("Error adding product rating:", { error });
    const err = error as Error & { code?: number };
    if (err.code === 11000) {
      errorResponse(res, err, "You have already rated this product", StatusCodes.CONFLICT);
    } else {
...
export async function deleteProductRating(req: Request, res: Response): Promise<void> {
  try {
    const authReq = req as AuthenticatedRequest;
    ...
    if (rating.ratedBy.toString() !== userId.toString() && (authReq.user as { role?: string }).role !== "admin") {
      errorResponse(res, null, "You can only delete your own ratings", StatusCodes.FORBIDDEN);
      return;
    }
    await rating.deleteOne();

    const remainingRatings = await rateModel.find({ productId: toObjectId(productId) });
    const avgRating = remainingRatings.length > 0 ? remainingRatings.reduce((sum, r) => sum + r.rating, 0) / remainingRatings.length : 0;
    await productModel.findByIdAndUpdate(productId, { $set: { avgRating: Math.round(avgRating * 10) / 10, ratingCount: remainingRatings.length } });
...
export async function getRecommendations(req: Request, res: Response): Promise<void> {
  try {
    ...
    const userProducts = await productModel.find({ _id: { $in: [...favoriteIds, ...purchasedIds] } }).select("category brand");
...
export async function getPopularProducts(req: Request, res: Response): Promise<void> {
  try {
    const limit = parseInt(req.query.limit as string) || 10;
    const products = await productModel.find({ status: "active" }).sort({ ratingCount: -1, avgRating: -1 }).limit(limit);
...
export async function getNewArrivals(req: Request, res: Response): Promise<void> {
  try {
    const limit = parseInt(req.query.limit as string) || 10;
    const products = await productModel.find({ status: "active" }).sort({ createdAt: -1 }).limit(limit);
